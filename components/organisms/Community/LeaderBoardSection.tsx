                    ...
                    <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                            ...
                            <View className="flex-row bg-zinc-800 rounded-xl p-1">
                                {["week", "month", "all"].map((period) => (
                                    <TouchableOpacity
                                        ...
                                        onPress={() => setTimeframe(period as "week" | "month" | "all")}
                                    >
                                        <Text
                        ...
                        <View className="px-4 mb-6">
                            <View className="flex-row justify-center items-end">
                                {/* 2nd Place */}
                                <TouchableOpacity className="items-center mx-2" onPress={() => onUserPress(MOCK_LEADERBOARD[1].user)}>
                                    <View className="w-16 h-16 rounded-full border-2 border-gray-400 overflow-hidden mb-2">
                                        <Image source={MOCK_LEADERBOARD[1].user.avatar} className="w-full h-full" />
                                    </View>
                                    <View className="bg-gray-400 rounded-lg px-3 py-6 items-center min-h-[60px]">
                                        <Text className="text-black font-bold text-lg">🥈</Text>
                                        <Text className="text-black text-xs font-bold text-center">{MOCK_LEADERBOARD[1].user.name}</Text>
                                        <Text className="text-black text-xs">{MOCK_LEADERBOARD[1].totalLikes}</Text>
                                    </View>
                                </TouchableOpacity>

                                {/* 1st Place */}
                                <TouchableOpacity className="items-center mx-2" onPress={() => onUserPress(MOCK_LEADERBOARD[0].user)}>
                                    <View className="w-20 h-20 rounded-full border-2 border-yellow-400 overflow-hidden mb-2">
                                        <Image source={MOCK_LEADERBOARD[0].user.avatar} className="w-full h-full" />
                                    </View>
                                    <View className="bg-yellow-400 rounded-lg px-3 py-8 items-center min-h-[80px]">
                                        ...
                                        <Text className="text-black text-sm font-bold text-center">{MOCK_LEADERBOARD[0].user.name}</Text>
                                        <Text className="text-black text-xs">{MOCK_LEADERBOARD[0].totalLikes}</Text>
                                    </View>
                                </TouchableOpacity>

                                {/* 3rd Place */}
                                <TouchableOpacity className="items-center mx-2" onPress={() => onUserPress(MOCK_LEADERBOARD[2].user)}>
                                    <View className="w-16 h-16 rounded-full border-2 border-amber-600 overflow-hidden mb-2">
                                        <Image source={MOCK_LEADERBOARD[2].user.avatar} className="w-full h-full" />
                                    </View>
                                    <View className="bg-amber-600 rounded-lg px-3 py-4 items-center min-h-[40px]">
                                        <Text className="text-black font-bold text-lg">🥉</Text>
                                        <Text className="text-black text-xs font-bold text-center">{MOCK_LEADERBOARD[2].user.name}</Text>
                                        <Text className="text-black text-xs">{MOCK_LEADERBOARD[2].totalLikes}</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                        </View>
